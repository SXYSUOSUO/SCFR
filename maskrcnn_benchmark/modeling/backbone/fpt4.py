...
class FPT_B(nn.Module):
    def __init__(self, feature_dim, window_size, position_emd = False, need_head=True,top_blocks =None, with_norm='none'):
        ...
        self.st_p2 = Swin_transformer(in_channels=feature_dim, out_channels=feature_dim, window_size=5, num_heads=2, position_emd = position_emd, bias=False)
        ...
        self.gt_p2_p3 = Swin_transformer_up(in_channels=feature_dim, out_channels=feature_dim, window_size=5, scale=2, num_heads=2, position_emd=False, bias=False)
        self.gt_p2_p4 = Swin_transformer_up(in_channels=feature_dim, out_channels=feature_dim, window_size=5, scale=4, num_heads=2, position_emd=False, bias=False)
        self.gt_p2_p5 = Swin_transformer_up(in_channels=feature_dim, out_channels=feature_dim, window_size=5, scale=8, num_heads=2, position_emd=False, bias=False)
        ...
        if with_norm != 'none':
            ...
            self.fpt_p2 = nn.Sequential(
                *[nn.Conv2d(feature_dim * 5, feature_dim, 1, padding=0, bias=False), norm(feature_dim)])
    ...
    def forward(self, features):  # 输入：fpn的2-5个阶段的特征图
        ...
        if self.need_head ==True:
            ...
            fpn_p2_1 = self.fpn_p2_1x1(features[0])
        ...
        fpt_p2_out = torch.cat([self.st_p2(fpn_p2_1,fpn_p2_1), self.gt_p2_p5(fpn_p2_1, fpn_p5_1), self.gt_p2_p4(fpn_p2_1, fpn_p4_1),self.gt_p2_p3(fpn_p2_1, fpn_p3_1), fpn_p2_1], 1)
        
        #pdb.set_trace()
        fpt_p5 = F.gelu(self.fpt_p5(fpt_p5_out))
        fpt_p4 = F.gelu(self.fpt_p4(fpt_p4_out))
        fpt_p3 = F.gelu(self.fpt_p3(fpt_p3_out)) 
        fpt_p2 = F.gelu(self.fpt_p2(fpt_p2_out)) 
        ...
        '''
        fpt_features = [fpt_p2, fpt_p3, fpt_p4, fpt_p5]
