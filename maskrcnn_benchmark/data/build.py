...
def build_dataset(dataset_list, transforms, dataset_catalog, is_together=False, is_train=True, is_source=True):
    """
    Arguments:
    ...
    if not is_train and not is_together:
        return datasets

    if is_together:
        return [D.MultiDataset(datasets)]
...
def make_data_loader(cfg, is_train=True, is_source=True, is_distributed=False, start_iter=0):
    ...
    is_together = cfg.DATASETS.TOGETHER
    datasets = build_dataset(dataset_list, transforms, DatasetCatalog, is_together, is_train, is_source)
