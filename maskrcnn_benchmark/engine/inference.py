...
def compute_on_dataset(model, data_loader, device,save_feature):
    model.eval()
    results_dict = {}
    results_dict1 = {}
    results_dict2 = {}
        ...
        if len(batch)==6:
            ...
            with torch.no_grad():
                ...
                if save_feature == True:
                    output1, output2, SMap_before1, SMap_before2, SMap_after1, SMap_after2  = model.forward(images1,images2)
                else:
                    output1,output2 = model.forward(images1,images2)
            ...
            if save_feature == True:
                results_SMB1.update({img_id: result for img_id, result in zip(image_ids, SMap_before1)})
                results_SMB2.update({img_id: result for img_id, result in zip(image_ids, SMap_before2)})

                results_SMA1.update({img_id: result for img_id, result in zip(image_ids, SMap_after1)})
                results_SMA2.update({img_id: result for img_id, result in zip(image_ids, SMap_after2)})
    ...
    if results_dict =={}:
        if save_feature == True:
            return [results_dict1, results_dict2, results_SMB1, results_SMB2, results_SMA1, results_SMA2]
...
def savemap(file,SM):
    ...
    for f_k in SM.keys():
        filename=os.path.join(file, f_k)
        if os.path.exists(filename) == False:
            os.mkdir(filename)
        torch.save(SM[f_k], os.path.join(filename, "map.pth"))
...
def inference(
    ...
    #pdb.set_trace()
    predictions = compute_on_dataset(model, data_loader, device, save_feature)
    ...
    if len(predictions)>1 and type(dataset) == datasets.multi_dataset.MultiDataset:
        if save_feature ==True:
            results_SM = [{'before':predictions[2],'after':predictions[4]},{'before':predictions[3],'after':predictions[5]}]
    ...
    if isinstance(dataset, datasets.MultiDataset):
        for i in range(0,len(dataset.datasets)):
            ...
            if save_feature ==True:
                savemap(output_folder[i],results_SM[i])
